...
<script>
    ...
    const EF3Insert_service = (ef3_orden_nvo,ef3_desc_nvo) => {
        return new Promise(function (resolve, reject) {
            const objXMLHttpRequest = new XMLHttpRequest();
            ...
            objXMLHttpRequest.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
            objXMLHttpRequest.send("ef3_orden="+ef3_orden_nvo+"&ef3_desc="+ef3_desc_nvo);
        });
    }

    const EF3Update_service = (ID,Ef3_orden_ant,Ef3_desc_ant,ID_EF_NVO,Ef3_desc_nvo) => {
        const ID_big = parseInt(ID);
        let data =
            JSON.stringify({
                id : ID_big,
                ef3_orden_ant : Ef3_orden_ant,
                ef3_desc_ant :  Ef3_desc_ant,
                ef3_orden_nvo : ID_EF_NVO,
                ef3_desc_nvo : Ef3_desc_nvo});
    ...
    const getEF3 = async () => {
        try{
            const response = await EF3_service();
            ...
            for(var i = 0; i < myArr.length; i++) {
                ...
                campo.textContent = ef3;
                campo.value = ef3;
                linea.appendChild(campo);

                var opt = myArr[i].EF3_Desc;
                ...
                editar.setAttribute("name", myArr[i].EF3_Desc);
                editar.setAttribute("id", myArr[i].EF3);
                editar.setAttribute("class", "btn btn-outline-warning px-3");
                editar.setAttribute("type", "button");
                editar.textContent = "Editar";
                editar.value = myArr[i].id_ef3;
                ...
                boton.setAttribute("name", myArr[i].id_ef3);
                boton.setAttribute("id", myArr[i].id_ef3);
    ...
    const insert_ef3 = (ef3_orden_nvo,ef3_desc_nvo) => {
        try {
            EF3Insert_service(ef3_orden_nvo, ef3_desc_nvo);
            Swal.fire({
                icon: 'success',
    ...
    const update_ef3 = (id,ef3_orden_ant, ef3_desc_ant,ID_EF_NVO,ef3_desc_nvo) => {
        try {
            EF3Update_service(id,ef3_orden_ant, ef3_desc_ant,ID_EF_NVO,ef3_desc_nvo);
            Swal.fire({
                icon: 'success',
                title: 'Registro Agregado',
                showConfirmButton: false,
...
<nav class="navbar navbar-expand-lg fixed-top navbar-white bg-white border-bottom" aria-label="Main navigation">
    <div class="container-fluid">
        ...
        <button class="btn btn-success mt-3" onclick="insert_ef3($('#IdDesc').val(),$('#Desc').val())">
            Agregar
        </button>
    </div>
</nav>
...
<main class="container" style="max-width:1420px;">
    <div class="my-3 p-4 bg-body rounded shadow-sm" id="panel">
        <div class="border-bottom d-flex flex-row">
            ...
            <h3 class="w-100  d-flex justify-content-left text-primary" id="titulo">Agrupación EF3</h3>
        </div>
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                ...
                <div class="modal-footer">
                    ...
                    <button type="button" class="btn btn-primary" onclick="EF3Update_service($('#postId').val(),$('#ID_EF_ANT').val(),$('#EF_name').val(),$('#ID_EF_NVO').val(),$('#EF_name_nvo').val())">Editar</button>
                </div>
            </div>
        </div>
    </div>
