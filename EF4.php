...
<script>
    ...
    const EF4_service = () => {
        return new Promise(function (resolve, reject) {
            ...
            objXMLHttpRequest.open('GET', 'http://localhost/Artigraf/ef4_controller.php');
            objXMLHttpRequest.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
            objXMLHttpRequest.send();
        });
    }

    const EF4Insert_service = (ef4_orden_nvo,ef4_desc_nvo) => {
        return new Promise(function (resolve, reject) {
            const objXMLHttpRequest = new XMLHttpRequest();
            ...
            objXMLHttpRequest.open('POST', 'http://localhost/Artigraf/ef4_controller.php');
            objXMLHttpRequest.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
            objXMLHttpRequest.send("ef4_orden="+ef4_orden_nvo+"&ef4_desc="+ef4_desc_nvo);
        });
    }

    const EF4Update_service = (ID,Ef4_orden_ant,Ef4_desc_ant,ID_EF_NVO,Ef4_desc_nvo) => {
        const ID_big = parseInt(ID);
        let data =
            JSON.stringify({
                id : ID_big,
                ef4_orden_ant : Ef4_orden_ant,
                ef4_desc_ant :  Ef4_desc_ant,
                ef4_orden_nvo : ID_EF_NVO,
                ef4_desc_nvo : Ef4_desc_nvo});
        ...
        return new Promise(function (resolve, reject) {
            ...
            objXMLHttpRequest.open('PUT', 'http://localhost/Artigraf/ef4_controller.php');
            objXMLHttpRequest.setRequestHeader("Content-type", "application/json");
            objXMLHttpRequest.send(data);
        });
    }

    const EF4Delete_service = (ID) => {
        ...
        return new Promise(function (resolve, reject) {
            ...
            objXMLHttpRequest.open('DELETE', 'http://localhost/Artigraf/ef4_controller.php');
            objXMLHttpRequest.setRequestHeader("Content-type", "application/json");
            objXMLHttpRequest.send(data);
        });
    }
    ...
    const getEF4 = async () => {
        try{
            const response = await EF4_service();
            ...
            for(var i = 0; i < myArr.length; i++) {
                ...
                campo.textContent = ef4;
                campo.value = ef4;
                linea.appendChild(campo);

                var opt = myArr[i].EF4_Desc;
                ...
                editar.setAttribute("name", myArr[i].EF4_Desc);
                editar.setAttribute("id", myArr[i].EF4);
                editar.setAttribute("class", "btn btn-outline-warning px-3");
                editar.setAttribute("type", "button");
                editar.textContent = "Editar";
                editar.value = myArr[i].id_ef4;
                ...
                boton.setAttribute("name", myArr[i].id_ef4);
                boton.setAttribute("id", myArr[i].id_ef4);
    ...
    const insert_ef4 = (ef4_orden_nvo,ef4_desc_nvo) => {
        try {
            EF4Insert_service(ef4_orden_nvo, ef4_desc_nvo);
            Swal.fire({
                icon: 'success',
    ...
    const update_ef4 = (id,ef4_orden_ant, ef4_desc_ant,ID_EF_NVO,ef4_desc_nvo) => {
        try {
            EF4Update_service(id,ef4_orden_ant, ef4_desc_ant,ID_EF_NVO,ef4_desc_nvo);
            Swal.fire({
                icon: 'success',
                title: 'Registro Agregado',
                showConfirmButton: false,
...
<nav class="navbar navbar-expand-lg fixed-top navbar-white bg-white border-bottom" aria-label="Main navigation">
    <div class="container-fluid">
        ...
        <button class="btn btn-success mt-3" onclick="insert_ef4($('#IdDesc').val(),$('#Desc').val())">
            Agregar
        </button>
    </div>
</nav>
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                ...
                <div class="modal-footer">
                    ...
                    <button type="button" class="btn btn-primary" onclick="EF4Update_service($('#postId').val(),$('#ID_EF_ANT').val(),$('#EF_name').val(),$('#ID_EF_NVO').val(),$('#EF_name_nvo').val())">Editar</button>
                </div>
            </div>
        </div>
    </div>
