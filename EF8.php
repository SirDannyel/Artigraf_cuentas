...
<script>
    ...
    const EF8Insert_service = (ef8_orden_nvo,ef8_desc_nvo) => {
        return new Promise(function (resolve, reject) {
            const objXMLHttpRequest = new XMLHttpRequest();
            ...
            objXMLHttpRequest.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
            objXMLHttpRequest.send("ef8_orden="+ef8_orden_nvo+"&ef8_desc="+ef8_desc_nvo);
        });
    }

    const EF8Update_service = (ID,Ef8_orden_ant,Ef8_desc_ant,ID_EF_NVO,Ef8_desc_nvo) => {
        const ID_big = parseInt(ID);
        let data =
            JSON.stringify({
                id : ID_big,
                ef8_orden_ant : Ef8_orden_ant,
                ef8_desc_ant :  Ef8_desc_ant,
                ef8_orden_nvo : ID_EF_NVO,
                ef8_desc_nvo : Ef8_desc_nvo});
    ...
    const getEF8 = async () => {
        try{
            const response = await EF8_service();
            ...
            for(var i = 0; i < myArr.length; i++) {
                ...
                campo.textContent = ef8;
                campo.value = ef8;
                linea.appendChild(campo);

                var opt = myArr[i].EF8_Desc;
                ...
                editar.setAttribute("name", myArr[i].EF8_Desc);
                editar.setAttribute("id", myArr[i].EF8);
                editar.setAttribute("class", "btn btn-outline-warning px-3");
                editar.setAttribute("type", "button");
                editar.textContent = "Editar";
                editar.value = myArr[i].id_ef8;
                ...
                boton.setAttribute("name", myArr[i].id_ef8);
                boton.setAttribute("id", myArr[i].id_ef8);
    ...
    const insert_ef8 = (ef8_orden_nvo,ef8_desc_nvo) => {
        try {
            EF8Insert_service(ef8_orden_nvo, ef8_desc_nvo);
            Swal.fire({
                icon: 'success',
    ...
    const update_ef8 = (id,ef8_orden_ant, ef8_desc_ant,ID_EF_NVO,ef8_desc_nvo) => {
        try {
            EF8Update_service(id,ef8_orden_ant, ef8_desc_ant,ID_EF_NVO,ef8_desc_nvo);
            Swal.fire({
                icon: 'success',
                title: 'Registro Agregado',
                showConfirmButton: false,
...
<nav class="navbar navbar-expand-lg fixed-top navbar-white bg-white border-bottom" aria-label="Main navigation">
    <div class="container-fluid">
        ...
        <button class="btn btn-success mt-3" onclick="insert_ef8($('#IdDesc').val(),$('#Desc').val())">
            Agregar
        </button>
    </div>
</nav>
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                ...
                <div class="modal-footer">
                    ...
                    <button type="button" class="btn btn-primary" onclick="EF8Update_service($('#postId').val(),$('#ID_EF_ANT').val(),$('#EF_name').val(),$('#ID_EF_NVO').val(),$('#EF_name_nvo').val())">Editar</button>
                </div>
            </div>
        </div>
    </div>
