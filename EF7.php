...
<script>
    ...
    const EF7Insert_service = (ef7_orden_nvo,ef7_desc_nvo) => {
        return new Promise(function (resolve, reject) {
            const objXMLHttpRequest = new XMLHttpRequest();
            ...
            objXMLHttpRequest.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
            objXMLHttpRequest.send("ef7_orden="+ef7_orden_nvo+"&ef7_desc="+ef7_desc_nvo);
        });
    }

    const EF7Update_service = (ID,Ef7_orden_ant,Ef7_desc_ant,ID_EF_NVO,Ef7_desc_nvo) => {
        const ID_big = parseInt(ID);
        let data =
            JSON.stringify({
                id : ID_big,
                ef7_orden_ant : Ef7_orden_ant,
                ef7_desc_ant :  Ef7_desc_ant,
                ef7_orden_nvo : ID_EF_NVO,
                ef7_desc_nvo : Ef7_desc_nvo});
    ...
    const getEF7 = async () => {
        try{
            const response = await EF7_service();
            ...
            for(var i = 0; i < myArr.length; i++) {
                ...
                campo.textContent = ef7;
                campo.value = ef7;
                linea.appendChild(campo);

                var opt = myArr[i].EF7_Desc;
                ...
                editar.setAttribute("name", myArr[i].EF7_Desc);
                editar.setAttribute("id", myArr[i].EF7);
                editar.setAttribute("class", "btn btn-outline-warning px-3");
                editar.setAttribute("type", "button");
                editar.textContent = "Editar";
                editar.value = myArr[i].id_ef7;
                ...
                boton.setAttribute("name", myArr[i].id_ef7);
                boton.setAttribute("id", myArr[i].id_ef7);
    ...
    const insert_ef7 = (ef7_orden_nvo,ef7_desc_nvo) => {
        try {
            EF7Insert_service(ef7_orden_nvo, ef7_desc_nvo);
            Swal.fire({
                icon: 'success',
    ...
    const update_ef7 = (id,ef7_orden_ant, ef7_desc_ant,ID_EF_NVO,ef7_desc_nvo) => {
        try {
            EF7Update_service(id,ef7_orden_ant, ef7_desc_ant,ID_EF_NVO,ef7_desc_nvo);
            Swal.fire({
                icon: 'success',
                title: 'Registro Agregado',
                showConfirmButton: false,
...
<nav class="navbar navbar-expand-lg fixed-top navbar-white bg-white border-bottom" aria-label="Main navigation">
    <div class="container-fluid">
        ...
        <button class="btn btn-success mt-3" onclick="insert_ef7($('#IdDesc').val(),$('#Desc').val())">
            Agregar
        </button>
    </div>
</nav>
...
<main class="container" style="max-width:1420px;">
    <div class="my-3 p-4 bg-body rounded shadow-sm" id="panel">
        <div class="border-bottom d-flex flex-row">
            ...
            <h3 class="w-100  d-flex justify-content-left text-primary" id="titulo">Agrupación EF7</h3>
        </div>
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                ...
                <div class="modal-footer">
                    ...
                    <button type="button" class="btn btn-primary" onclick="EF7Update_service($('#postId').val(),$('#ID_EF_ANT').val(),$('#EF_name').val(),$('#ID_EF_NVO').val(),$('#EF_name_nvo').val())">Editar</button>
                </div>
            </div>
        </div>
    </div>
